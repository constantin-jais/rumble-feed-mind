...
The protected-branch profile passes with zero parser errors, zero unknown state, complete enabled/forced RLS coverage and no waiver. CI downloads the immutable `wrench-db-inspect` Linux archive from the consolidated Proof Kit release `db-inspect-v0.1.0-alpha.5` and verifies SHA-256 before producing JSON and Markdown evidence.
